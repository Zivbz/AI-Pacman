...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        # don't prefer to stop
        if action == 'Stop':
            return -999999

        # in case we're having a ghost on the next state
        for state in newGhostStates:
            if state.getPosition() == newPos:
                if state.scaredTimer == 0:  # if it's not scared - get away
                    return -999999
                else:  # if it's scared -> eat it!
                    return 999999

        if newFood.count() == 0:  # if there's no food left, finish
            return 0

            # in case we're eating food in this turn
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def miniMax(gameState, agent, depth):
            ...
            # Available ghosts. Pick next ghost #
            else:
                nextAgent = agent + 1
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def AB(gameState, agent, depth, a, b):
            ...
            # All ghosts have finished one round: increase depth #
            if agent == gameState.getNumAgents() - 1:
                depth += 1
            ...
            for action in gameState.getLegalActions(agent):
                if not result:  # First move
                    ...
                    # Check if we can overpass some nodes                   #
...
def betterEvaluationFunction(currentGameState):
    ...
    # Score tell us some information about current state  #
